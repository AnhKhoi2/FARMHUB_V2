...
export const aiController = {
  ...
  chat: asyncHandler(async (req, res) => {
    ...
    try {
      ...
      // Persist chat session (store the exchange). We'll store the full message array
      try {
        const savedMessages = (messages || []).map(m => ({ role: (m.role || 'user'), text: m.content || m.text || '' }));
        // append assistant response
        savedMessages.push({ role: 'assistant', text: result?.text || '', cause: result?.cause || '', treatment: result?.treatment || '', provider: result?.provider || 'gemini' });

        await AIChatSession.create({ user: req.user?.id, messages: savedMessages, provider: result?.provider || 'gemini' });
      } catch (e) {
        console.warn('aiController.chat: failed to save chat session', e && e.message);
      }
      ...
      try {
        await AIChatSession.create({ user: req.user?.id, messages: [{ role: 'user', text: (messages?.slice(-1)?.[0]?.content || '') }, { role: 'assistant', text: mockText, provider: 'mock' }], provider: 'mock' });
      } catch (e) {
        console.warn('aiController.chat: failed to save mock chat session', e && e.message);
      }
