...
export default function AdminLayout({ children }) {
  ...
  return (
    <div className="d-flex" style={{ minHeight: "100vh" }}>
      <aside className="p-3 text-white" style={{ width: 260, background: "linear-gradient(180deg,#0d6efd 0%, #6f42c1 100%)" }}>
        <div className="d-flex align-items-center mb-4">
          <div className="fs-4 fw-bold">FarmHub Admin</div>
        </div>

        <nav className="nav flex-column mb-3">
          <NavLink to="/admin/dashboard" className={({isActive})=>"nav-link text-white py-2" + (isActive? ' active bg-white bg-opacity-10 rounded':'')}>Dashboard</NavLink>
