...
export const verifyToken = asyncHandler((req, _res, next) => {
  if (isBypass()) {
    // ⚠️ Bypass: gắn user giả có quyền admin
  ...
  jwt.verify(token, process.env.JWT_ACCESS_KEY, (err, payload) => {
    ...
    // ⭐ Chuẩn hóa payload → luôn có req.user.id
    const normalized = {
    ...
    if (!normalized.id) {
      throw Unauthorized("Token không hợp lệ: thiếu user id.");
    }
...
// ⭐ Alias cho code cũ đang dùng tên `protect`
...
export const requireRoles = (roles = []) => (req, _res, next) => {
  ...
  next(
    Forbidden(
      `Bạn không có đủ quyền (${requiredRoles.join(
        ", "
...
export const requireOwnerOrAdmin = (param = "id") => (req, _res, next) => {
  ...
  const targetId =
    req.validated?.params?.[param] ?? req.params?.[param];

  if (!targetId)
    return next(
      Forbidden(
        `Lỗi cấu hình middleware: Thiếu ID (${param}) trong request.`
      )
    );
  ...
  if (
    req.user?.role === "admin" ||
    String(userIdFromToken) === String(targetId)
  ) {
    return next();
  }
  ...
  return next(
    Forbidden(
      "Bạn chỉ có thể thực hiện thao tác này trên tài khoản của chính mình hoặc phải là Admin"
    )
  );
};
