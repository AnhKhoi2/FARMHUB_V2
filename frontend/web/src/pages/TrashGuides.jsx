...
export default function TrashGuides(){
  ...
  const fetchTrash = useCallback(async (p = page) => {
    ...
    setError(null);
    try{
      const res = await axiosClient.get('/guides/trash', { params: { page: p, limit } });
      const data = res.data || {};
  ...
  async function onRestore(id){
    ...
    try{
      ...
      fetchTrash(page);
    }catch(e){ console.error(e); alert('Khôi phục thất bại'); }
    finally{ setLoading(false); }
  }
  ...
  async function onPermanentDelete(id){
    ...
    try{
      ...
      if (remaining <= 0 && page > 1){ setPage(page - 1); fetchTrash(page - 1); }
      else fetchTrash(page);
    }catch(e){ console.error(e); alert('Xóa vĩnh viễn thất bại'); }
    finally{ setLoading(false); }
  }
    ...
    <div className="manager-guides-page">
      <header className="mg-header">
        ...
        <div style={{display:'flex',gap:8}}>
          <button className="mg-btn" onClick={() => navigate('/managerguides')}>Quay lại danh sách</button>
        </div>
      </header>

      <div className="mg-grid-container">
        {loading ? (<div className="mg-loading">Đang tải...</div>) : guides.length === 0 ? (
          <div className="mg-empty">Không có hướng dẫn đã xóa.</div>
        ) : (
          <div className="mg-grid">
            {guides.map(g => (
              <div className="mg-card" key={g._id || g.id}>
                <div className="mg-thumb" style={{ backgroundImage: `url(${g.image || placeholderImg})` }} />
                <div className="mg-card-body">
                  <div className="mg-card-title">{g.title}</div>
                  <div className="mg-card-sub">{g.description || g.summary || '—'}</div>
                </div>
                <div className="mg-card-actions">
                  <button className="mg-btn" onClick={()=> onRestore(g._id || g.id)}>Khôi phục</button>
                  <button className="mg-btn mg-btn-delete" onClick={()=> onPermanentDelete(g._id || g.id)}>Xóa vĩnh viễn</button>
                </div>
              </div>
            ))}
          </div>
