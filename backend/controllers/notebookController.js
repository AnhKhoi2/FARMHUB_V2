...
/**
 * Tính số ngày từ startDate đến endDate (theo giờ Việt Nam UTC+7)
 * Sử dụng helper từ timezone.js
 */
const getDaysDifference = getDaysDifferenceVN;

/**
 * Tính stage end date dựa trên planted_date và stage.day_end (theo giờ Việt Nam UTC+7)
 */
...
/**
 * Tính ngày bắt đầu của stage dựa trên planted_date và stage.day_start
 */
...
/**
 * Lấy template phù hợp với notebook (dựa trên guide's plant_group)
 */
...
const assignTemplateToNotebook = async (notebookId, templateId) => {
  ...
  if (!template) {
    throw new AppError("Không tìm thấy template", 404);
  }
...
/**
 * Tạo daily checklist từ template's autogenerated_tasks
 */
export const generateDailyChecklist = async (notebookId) => {
  ...
  // ✅ Kiểm tra notebook đã hoàn thành 100% -> không tạo task mới
  ...
  if (isNotebookCompleted) {
    console.log(
      `✅ Notebook ${notebookId} đã hoàn thành 100% - không tạo task mới`
    );
...
const updateCurrentStage = async (notebookId, newStageNumber) => {
  ...
  if (!notebook || !notebook.template_id) {
    throw new AppError("Không tìm thấy nhật ký hoặc template", 404);
  }

  const template = notebook.template_id;

  if (newStageNumber > template.stages.length) {
    throw new AppError("Số stage không hợp lệ", 400);
  }
...
/**
 * Kiểm tra và xử lý trạng thái stage của notebook (stage monitoring)
 */
export const checkNotebookStageStatus = async (notebook) => {
  try {
    ...
    if (!notebook.template_id || !notebook.template_id.stages) {
      console.log(`⚠️ Notebook ${notebook._id} không có template`);
      return;
    }
    ...
    if (!currentStageTracking) {
      console.log(
        `⚠️ Không tìm thấy stage_tracking cho stage ${notebook.current_stage}`
      );
      return;
    }
    ...
    if (
      ...
      console.log(
        `✅ Stage ${notebook.current_stage} đã ${currentStageTracking.status}, bỏ qua`
      );
      return;
    }
    ...
    if (!templateStage) {
      console.log(
        `⚠️ Không tìm thấy template stage cho stage ${notebook.current_stage}`
      );
      return;
    }
    ...
    console.log(
      `📅 Notebook: ${notebook.notebook_name} | Stage: ${
        templateStage.name
      } | End Date: ${
...
const completeChecklistTask = async (notebookId, taskName) => {
  ...
  if (currentStageTracking) {
    ...
    if (!dailyLog) {
      ...
      console.log(
        `✅ Found existing daily_log for ${today}: ${dailyLog.daily_progress}%`
      );
    }
    ...
    if (totalTodayTasks > 0) {
      ...
      console.log(
        `📅 Daily progress for ${today}: ${dailyLog.daily_progress}% (${completedTodayTasks}/${totalTodayTasks})`
      );
    }
  } else {
    console.warn(
      `⚠️ No currentStageTracking found for stage ${notebook.current_stage}`
    );
  }
...
/**
 * Lấy tất cả observations của stage hiện tại
 * CHỈ trả về observations nếu đang ở ngày cuối của giai đoạn
 */
const getCurrentStageObservations = async (notebookId) => {
  ...
  if (stageTracking && stageTracking.status === "completed") {
    // Trả về danh sách observation đã ghi nhận (có value=true)
    return (
...
/**
 * Cập nhật observation cho stage hiện tại
 */
export const updateStageObservation = async (
  ...
  // ✅ Kiểm tra xem tất cả observations đã hoàn thành chưa
  if (notebook.template_id && notebook.template_id.stages) {
    ...
    if (currentTemplateStage && currentTemplateStage.observation_required) {
      ...
      console.log(
        `📊 Observations: ${completedObservations.length}/${requiredObservations.length} completed`
      );

      // Nếu tất cả observations bắt buộc đã hoàn thành (value = true)
      if (completedObservations.length >= requiredObservations.length) {
        console.log(
          `✅ All observations completed for stage ${notebook.current_stage}`
        );

        // Đánh dấu stage hiện tại hoàn thành (sử dụng template-based end date)
        const currentTemplateStage = notebook.template_id.stages.find(
          (s) => s.stage_number === notebook.current_stage
        );
        ...
        // 🎉 KIỂM TRA HOÀN THÀNH NOTEBOOK 100%
        if (notebook.progress === 100) {
          ...
          if (allStagesCompleted) {
            console.log(
              `🎊 Notebook ${notebook.notebook_name} hoàn thành 100%! Gửi thông báo...`
            );

            // Gửi thông báo hoàn thành notebook
            await sendNotebookCompletedNotification({
              userId: notebook.user_id,
              notebookId: notebook._id,
              notebookName: notebook.notebook_name,
              plantType: notebook.plant_type || "cây",
...
/**
 * Tính stage hiện tại dựa trên số ngày đã trồng
 */
...
// ♻️ Khôi phục notebook đã xóa
export const restoreNotebook = asyncHandler(async (req, res) => {
  const { id } = req.params;

  // Kiểm tra notebook có tồn tại và đã bị xóa không
  const notebook = await Notebook.findOne({
    _id: id,
    user_id: req.user.id,
    status: "deleted",
...
// 🗑️ Lấy danh sách notebook đã xóa
...
// 💀 Xóa vĩnh viễn notebook (hard delete)
export const permanentDeleteNotebook = asyncHandler(async (req, res) => {
  ...
  const notebook = await Notebook.findOneAndDelete({
    _id: id,
    user_id: req.user.id,
    status: "deleted", // Chỉ cho phép xóa vĩnh viễn notebook đã ở trạng thái deleted
  });
...
export const getNotebookTimeline = asyncHandler(async (req, res) => {
  ...
  if (!notebook || !notebook.template_id) {
    return ok(
      ...
      null,
      { hasTemplate: false },
      "Notebook chưa có template. Vui lòng gán template để xem timeline."
    );
  }
  ...
  // Thêm ngày hiển thị (dựa trên planted_date + template day_start/day_end)
...
export const updateObservation = asyncHandler(async (req, res) => {
  ...
  if (currentStage && currentStage.stage_number > previousStageNumber) {
    return ok(
      ...
      `✅ Đã hoàn thành tất cả điều kiện quan sát! Tự động chuyển sang ${currentStage.stage_name}.`
    );
  }
...
export const migrateNotebooks = asyncHandler(async (req, res) => {
  ...
  for (const notebook of notebooks) {
    ...
    if (needsSave) {
      ...
      migratedCount++;
      console.log(
        `✅ Migrated notebook: ${notebook.notebook_name} (${notebook._id})`
      );
    }
  }
...
// 🔍 Kiểm tra một notebook cụ thể (manual trigger)
...
// 🔄 Monitor tất cả notebooks (admin/cron endpoint)
...
// 📅 Lấy trạng thái hàng ngày (tasks hôm nay + overdue summary)
export const getDailyStatus = asyncHandler(async (req, res) => {
  ...
  console.log(
    `🔔 getDailyStatus called for notebook=${id} user=${req.user?.id}`
  );

  // Tạo checklist cho hôm nay (sẽ tự động xử lý overdue nếu sang ngày mới)
  ...
  // ✅ Kiểm tra notebook đã hoàn thành 100% -> không trả overdue summary
  const isNotebookCompleted =
...
export const skipOverdueTasks = asyncHandler(async (req, res) => {
  ...
  // Đánh dấu tất cả overdue tasks (được lưu trong stageTracking.overdue_tasks) = skipped
  const currentStageTracking = notebook.stages_tracking.find(
    (s) => s.stage_number === notebook.current_stage
  );
...
// ✅ Hoàn thành một overdue task (hoàn thành bù)
...
// ✅ Hoàn thành nhiều overdue tasks cùng lúc
...
export const getOverdueDetail = asyncHandler(async (req, res) => {
  ...
  console.log(
    `🔔 getOverdueDetail called for notebook=${id} user=${req.user?.id}`
  );
